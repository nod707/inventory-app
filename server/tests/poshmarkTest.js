...
async function testPoshmarkScraping() {
  console.log('Starting Poshmark scraping test...');
  ...
  try {
    ...
    const searchResults = await scraper.searchPoshmark(searchQuery);
    ...
    if (searchResults.length > 0) {
      ...
      if (searchResults[0]) {
        ...
        const itemDetails = await scraper.getPoshmarkItemDetails(searchResults[0].url);
        
        console.log('\nItem Details:');
        console.log('Title:', itemDetails.title);
        console.log('Price:', itemDetails.price);
        console.log('Brand:', itemDetails.brand);
        console.log('Size:', itemDetails.size);
...
// Run the test
testPoshmarkScraping().catch(console.error);
